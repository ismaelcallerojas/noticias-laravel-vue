<template>
    <div>
		...
		<div class="lg:flex justify-between items-center mb-6">
            <p class="text-2xl font-semibold mb-2 lg:mb-0">Categorias</p>
            <!-- Use v-if to check if user is premitted to add new category -->
			<Button size="large" @click="addModal=true" v-if="isWritePermitted"><Icon type="md-add" /> Agregar Categoria</Button>
        </div>
		...
		<section class="text-gray-700 w-full bg-white border rounded-lg p-4">
			<table class="min-w-full table-auto border rounded-lg">
				...
				<tr class="bg-gray-700">
					<th class="px-3 hidden sm:block py-2"><span class="text-gray-300">Icono</span></th>
					<th class="px-3 py-2"><span class="text-gray-300">Id</span></th>
					<th class="px-3 py-2"><span class="text-gray-300">Nombre</span></th>
					<th class="px-3 py-2 hidden md:table-cell"><span class="text-gray-300">Creado el</span></th>					
					<th class="px-2 py-2 w-5" v-show="isUpdatePermitted || isDeletePermitted"><span class="text-gray-300">Acciones</span></th>
				</tr>
				</thead>
		...
		<Modal
			...
			<Input v-model="data.categoryName" placeholder="Nombre de la Categoria"  />
			<Upload
				ref="uploads"
				type="drag"
				...
				<div class="py-5">
					...
					<p>Haga clic o arrastre la foto aquí para cargarlo</p>
				</div>
			</Upload>
			...
			<div slot="footer">
				...
				<Button type="primary" @click="addCategory" :disabled="isAdding" :loading="isAdding">{{isAdding ? 'Agregando..' : 'Agregar Categoria'}}</Button>
			</div>
		</Modal>
		<!-- Edit category modal-->
		<Modal
			...
			:closable="false"
		>
			<Input v-model="editData.categoryName" placeholder="Nombre de la Categoria"  />
			...
			<div class="img-upload" v-if="editData.iconImage">
				...
				<div slot="footer">
					<Button type="default" @click="closeEditModal">Cerrar</Button>
					<Button type="primary" @click="editCategory" :disabled="isAdding" :loading="isAdding">{{isAdding ? 'Editando..' : 'Guardar Cambios'}}</Button>
				</div>
		</Modal>
...
export default {
	...
	methods : {
		async addCategory(){
			if(this.data.categoryName.trim()=='') return this.e('El nombre de la Categoria es obligatorio');
			if(this.data.iconImage.trim()=='') return this.e('Debe agregar una imagen');
            
			const res = await this.callApi('post', '/app/create_category', this.data);
			if(res.status===201){
				this.$store.state.categories.unshift(res.data);
				this.s('¡La categoría ha sido añadida con éxito!');
				this.addModal = false;
		...
		async editCategory(){
			if(this.editData.categoryName.trim()=='') return this.e('El nombre de la Categoria es obligatorio');
			if(this.editData.iconImage.trim()=='') return this.e('Debe agregar una imagen');
			...
			if(res.status===200){
				...
				this.s('¡La categoría ha sido actualizada con éxito!');
		...
		showDeletingModal(category, i){
			this.deletingIndex = i
		 	const deleteModalObj  =  {
				...
				deletingIndex: i, 
				isDeleted : false,
				msg : '¿Está seguro de que desea eliminar esta categoría?',
        		successMsg: '¡La categoría ha sido eliminada con éxito!'
			}
