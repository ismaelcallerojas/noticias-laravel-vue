<template>
    <div>
		...
		<div class="lg:flex justify-between items-center mb-6">
            <p class="text-2xl font-semibold mb-2 lg:mb-0">Gestión de roles</p>

			<Button size="large" @click="addModal=true" v-if="isWritePermitted"><Icon type="md-add" /> Agregar un nuevo rol</Button>
        </div>
		<!-- Table - Roles -->
			...
			<table class="min-w-full table-auto border rounded-lg">
				<thead class="text-left">
					<tr class="bg-gray-700">
						...
						<th class="px-3 py-2"><span class="text-gray-300">Tipo de rol</span></th>
...
export default {
	...
	methods : {
		async add(){
			if(this.data.roleName.trim()=='') return this.e('El nombre del rol es obligatorio');
			const res = await this.callApi('post', '/app/create_role', this.data);
			if(res.status===201){
				this.$store.state.roles.unshift(res.data);
				this.s('¡El rol se ha agregado con éxito!');
				this.addModal = false;
				this.data.roleName = '';
			}else{
				if(res.status==422){
		...
		async edit(){
			if(this.editData.roleName.trim()=='') return this.e('El nombre del rol es obligatorio');
			...
			if(res.status===200){
				...
				this.s('¡El nombre del rol se ha actualizado correctamente!');
		...
		showEditModal(role, index){
	  	...
	  	showDeletingModal(data, i){
			...
			const deleteModalObj  =  {
				...
				deletingIndex: i,
				isDeleted : false,
				msg : '¿Está seguro de que desea eliminar este rol?',
				successMsg: '¡El rol se ha eliminado con éxito!'
			}
