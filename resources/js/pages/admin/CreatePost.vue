<template>
	<div>
		...
		<div class="lg:flex justify-between items-center mb-6">
            <p class="text-2xl font-semibold mb-2 lg:mb-0">Escribir una Noticia</p>
        </div>

		<div v-if="isWritePermitted">
			<div class="input-field">
				<Input type="text" v-model="data.title" placeholder="Título" />
			</div>
			...
			<Upload 
				...
				<div class="py-3">
					...
					<p class="text-gray-400">Subir imagen de portada</p>
				</div>
			</Upload>
			<div class="input-field" v-if="data.featuredImage">
				...
				<span class="inline-block text-gray-700 align-top pt-5">Cambiar imagen de portada</span>
			</div>
			...
			<div class="input-field">
				<Input  type="textarea" v-model="data.postExcerpt" :rows="4" placeholder="Extracto de la noticia " />
				</div>
			<div class="input-field">
				<Select  filterable multiple placeholder="Seleccionar Categoria" v-model="data.category_id">
			...
			<div class="input-field">
				<Button size="large" @click="save" :loading="isCreating" :disabled="isCreating"><Icon type="md-create" /> {{isCreating ? 'Espere por favor...' : 'Crear Noticia'}}</Button>
			</div>
		</div>    
    </div>
</template>
...
export default {
    ...
    async onSave(response){
		...
		if(this.data.title.trim()=='') return this.e('El título de la publicación es obligatorio');
		if(this.data.post.trim()=='') return this.e('El cuerpo de la publicación es obligatorio');
		if(this.data.postExcerpt.trim()=='') return this.e('Se requiere extracto de la noticia');
		if(this.data.metaDescription.trim()=='') return this.e('Se requiere meta description');
		if(!this.data.tag_id.length) return this.e('Se requiere etiqueta');
		if(!this.data.category_id.length) return this.e('La categoria es requerida');
			
		this.isCreating = true;
		const res = await this.callApi('post', '/app/create_post', this.data);
		if(res.status===200){
			this.s('¡LA noticia ha sido creado con éxito!');		 
	...
	async deleteImage(){
		...
		this.$refs.uploads.clearFiles(); // ViewUI mehtod - borra todos los datos del campo de entrada de carga
