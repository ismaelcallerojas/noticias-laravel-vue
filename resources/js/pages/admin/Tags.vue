...
<div>
	...
	<div class="lg:flex justify-between items-center mb-6">
		<p class="text-2xl font-semibold mb-2 lg:mb-0">Etiquetas</p>
		<!-- v-if="isWritePermitted" Check if user is premitted to add new tag -->
		<Button size="large" @click="addModal=true" v-if="isWritePermitted" ><Icon type="md-add" /> Agregar Etiqueta</Button>
	</div>
    ...
    <Modal
		...
		:mask="true"
		>
		<Input v-model="data.tagName" placeholder="Nombre de la etiqueta"  />

		<div slot="footer">
			<Button type="default" @click="addModal=false">Cerrar</Button>
			<Button type="primary" @click="addTag" :disabled="isAdding" :loading="isAdding">{{isAdding ? 'Agregando..' : 'Agregar Etiqueta'}}</Button>
		</div>
    </Modal>
    ...
    <Modal
		...
		:closable="false"
		>
		<Input v-model="editData.tagName" placeholder="Nuevo nombre"  />

		<div slot="footer">
			<Button type="default" @click="editModal=false">Cerrar</Button>
			<Button type="primary" @click="editTag" :disabled="isAdding" :loading="isAdding">{{isAdding ? 'Editando..' : 'Guardar Cambios'}}</Button>
		</div>
    </Modal>
  ...
  components : {
	...
	methods : {
		async addTag(){
			...
			if(this.data.tagName.trim()=='') return this.e('El nombre de la etiqueta es requerido');
			const res = await this.callApi('post', '/app/create_tag', this.data);
			if(res.status===201){
				this.$store.state.tags.unshift(res.data);
				this.s('La etiqueta se ha agregado con éxito!');
				this.addModal = false;
				this.data.tagName = '';
			}else{
		...
		async editTag(){
			if(this.editData.tagName.trim()=='') return this.e('El nombre de la etiqueta es requerido');
			...
			if(res.status===200){
				...
				this.s('La etiqueta se ha editado correctamente!');
		...
		showDeletingModal(tag, i){ 
			...
			const deleteModalObj  =  {
				...
				deletingIndex: i,
				isDeleted : false,
				msg : '¿Está seguro que quiere eliminar esta etiqueta?',
				successMsg: '¡La etiqueta ha sido eliminada con éxito!'
			}
