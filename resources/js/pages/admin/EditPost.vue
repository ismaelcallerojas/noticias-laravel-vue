<template>
    <div>
		...
		<div class="lg:flex justify-between items-center mb-6">
            <p class="text-2xl font-semibold mb-2 lg:mb-0">Actualizar Noticia</p>
        </div>
		...
		<div class="input-field"  v-if="data.featuredImage">
			...
			<span class="inline-block text-gray-700 align-top pt-5">Subir imagen de portada</span>
		</div>
		...
		<div class="input-field">
				<Input  type="textarea" v-model="data.postExcerpt" :rows="4" placeholder="Extracto de la publicación " />
			</div>
		<div class="input-field">
			<Select  filterable multiple placeholder="Selecciona una categoría" v-model="data.category_id">
				<Option v-for="(c, i) in getCategories" :value="c.id" :key="i">{{ c.categoryName }}</Option>
			</Select>
		</div>
		<div class="input-field">
			<Select  filterable multiple placeholder="Seleccionar etiqueta" v-model="data.tag_id">
		...
		<div class="input-field">
			<Button size="large" @click="save" :loading="isCreating" :disabled="isCreating"><Icon type="md-create" /> {{isCreating ? 'Espere por favor...' : 'Guardar Cambios'}}</Button>
		</div>
...
export default {
	...
	methods : {
		async onSave(response){
			...
			if(this.data.postExcerpt.trim()=='') return this.e('Se requiere extracto posterior');
			...
			if(res.status===200){
				this.s('¡La publicación ha sido actualizada con éxito!');
