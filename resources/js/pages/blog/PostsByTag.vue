...
<div class="min-h-screen flex flex-col">
    ...
    <!-- Texto del Header-->
    <blog-header :subtitle="`Etiqueta : ${tagName}`"></blog-header>
...
export default {
    data(){
        return {
            isLoggedIn : false,
            total: this.$store.state.total, // paginación - publicaciones por página
            pageInfo: {},
            sortedByTag: [],
            tagName: ''
        }
	...
	methods: {
		async getPostData(page=1) {
			...
			if(res.status==200){
				this.sortedByTag = res.data.posts.data; // obtener datos paginados
                this.pageInfo = res.data.posts; //  obtener el número de página/enlaces a anterior/siguiente
