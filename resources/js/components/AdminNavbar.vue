<template>
  <div class="sticky top-0 z-40">
    <div class="w-full h-20 px-6 bg-gray-100 border-b flex items-center justify-between">
      ...
      <div class="flex">
        ...
        <div class="inline-block lg:hidden flex items-center mr-4">
          ...
          <router-link to='/'>
              <p class="font-semibold text-lg sm:text-2xl text-blue-500 pl-4">&lt;&nbsp;Noticias UAB&nbsp;&gt;</p>
          </router-link>
        </div> 
      ...
      <div class="flex items-center relative cursor-pointer">
        ...
        <!--  Si la usuario no está logueado-->
    ...
    <div class="absolute bg-gray-100 border border-t-0 shadow-xl text-gray-700 rounded-b-lg w-48 bottom-10 right-0 mr-6" :class="dropDownOpen ? '' : 'hidden'">
      <router-link to="/" class="block px-4 py-2 hover:bg-gray-200" >Inicio</router-link>
      <!-- Login -->
      <router-link to='/login' class="block px-4 py-2 hover:bg-gray-200" v-if="!authUser()">Iniciar Sesión</router-link>           
      <!--  Si el usuario es admin-->
      <router-link to="/admin-redirect" class="block px-4 py-2 hover:bg-gray-200" v-if="authUser() && authUser().role.isAdmin">Administrar</router-link>
      <!-- Avatar -->
      ...
      <!--  Si la usuario está logueado-->
      <a href="/" @click="logout()" class="block px-4 py-2 hover:bg-gray-200" v-if="getUser">Cerrar Sesión</a>
    </div>
  </div>
</template>
