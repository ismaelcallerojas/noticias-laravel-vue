    ...
    <div class="bg-white rounded-lg overflow-hidden shadow relative">
        <router-link :to="`/post/${post.slug}`">
            ...
            <div v-else class="w-full h-48 bg-blue-700 flex">
                <span class="font-semibold text-lg text-white m-auto cursor-pointer">&lt;&nbsp;Noticias UAB&nbsp;&gt;</span>
            </div>
        </router-link>
        <div class="p-4 mt-2">
        ...
        <div class="p-4 flex items-center border-t border-gray-300" >
            ...
            <h2 class="text-sm tracking-tighter text-gray-900">
                <router-link :to="`/user/${post.user.name}/${post.user.id}`">Por {{post.user.name}}</router-link> 
