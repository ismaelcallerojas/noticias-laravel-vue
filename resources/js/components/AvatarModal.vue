<template>
    ...
    <Modal
        ...
        <span class="inline-block text-gray-700 align-top pt-5" v-if="avatar">Cargar imagen del avatar</span>

        <div slot="footer">
            <Button type="default" @click="$store.state.avatarModal=false">Cerra</Button>
            <Button type="primary" @click="upload" :disabled="isAdding" :loading="isAdding">{{isAdding ? 'Adding..' : 'Cambiar Avatar'}}</Button>
        </div>
...
export default {
    ...
    methods: {
        async upload(){     
            if(this.avatar.trim()=='') return this.e('No selecciono una imagen');
			const res = await this.callApi('post', '/app/update_avatar', {'avatar':this.avatar});
			if(res.status===200){
				this.s('Avatar actualizado!');
